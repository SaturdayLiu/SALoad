...
#import "SALConditionHandler.h"



@implementation SALConditionHandler
...
-(BOOL)canLoadTopWhenEndDrag{
    if ([self canLoadWhenEndDragWithLoadView:self.loadTopView direction:SALDirection_Vertical]) {
...
-(BOOL)canLoadBottomWhenEndDrag{
    if ([self canLoadWhenEndDragWithLoadView:self.loadBottomView direction:SALDirection_Vertical]) {
...
-(BOOL)canLoadLeftWhenEndDrag{
    if ([self canLoadWhenEndDragWithLoadView:self.loadLeftView direction:SALDirection_Horizontal]) {
...
-(BOOL)canLoadRightWhenEndDrag{
    if ([self canLoadWhenEndDragWithLoadView:self.loadRightView direction:SALDirection_Horizontal]) {
...
//没判断位移
-(BOOL)canLoadWhenEndDragWithLoadView:(LoadView *)loadView direction:(SALDirection)direction{
    if (!loadView) {
        return NO;
    }
...
//只判断速度
-(BOOL)canLoadWhenEndDragWithCurrentVelocityWithDirection:(SALDirection)direction{
    CGPoint velocityPoint = [self.scrollView.panGestureRecognizer velocityInView:self.scrollView];
    CGFloat velocity = direction == SALDirection_Horizontal ? velocityPoint.x : velocityPoint.y;
